    ...
    public static Request getRequest(MethodBuilder methodBuilder, File[] fils, String[] fileKeys, UIProgressRequestListener uiProgressRequestListener) {
        switch (methodBuilder.type()) {
            ...
            case Http.GET:
                ...
                return OkhttpUtil.getBuilder(methodBuilder, methodBuilder.request(), null).url(finalUrl).tag((methodBuilder.request() == null || methodBuilder.request().tag == null) ? methodBuilder.url() : methodBuilder.request().tag).cacheControl(OkHttpConfig.getInstance().cacheControl()).build();
            case Http.POST:
            case Http.PUT:
            case Http.DELETE:
            case Http.HEAD:
